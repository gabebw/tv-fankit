<?php
	...
	*/
	...
	if ( ! function_exists ('mysql_connect') ) die('<b>Fatal Error:</b> fkdb requires mySQL Lib to be compiled and or linked in to the PHP engine');
	if ( ! class_exists ('ezSQLcore') ) die('<b>Fatal Error:</b> fkdb requires ezSQLcore (ez_sql_core.php) to be included/loaded before it can be used');

	class fkdb extends ezSQLcore
	{
		...
		*  same time as initialising the fkdb class
		*/

		function fkdb($dbuser='', $dbpassword='', $dbname='', $dbhost='localhost')
		{
			$this->dbuser = $dbuser;
