...
QuoteEditor.prototype.startEditing = function(){
    ...
    this.deselect($('.line')); // Set lines to their base state.
    $.ajax({
...
QuoteEditor.prototype.setup_events = function(){
	...
	var elem = $(e.target).closest('div.line')[0];
	if( elem === undefined ){
...
QuoteEditor.prototype.add = function(elem){
	...
	tween.animate(
	    ...
	    400,
	    function(){ that.highlight($(this).removeAttr('style')); }
	);
    } 
};
...
QuoteEditor.prototype.remove = function(elem){
	...
	*/
	...
	tween = $.grep(tween, function(el, i){
	    return $.inArray(el, currentTween) == -1;
	});
    }
	...
	400,
	function(){ that.dehighlight($(this).removeAttr('style')); }
    );
    /* END UI stuff */
};
...
QuoteEditor.prototype.select = function(lines){
    $(lines).addClass('selected').removeClass('offedit');
};
...
QuoteEditor.prototype.deselect = function(lines){
    $(lines).removeClass('selected').addClass('offedit');
};
...
QuoteEditor.prototype.makeTween = function(idOne, idTwo){
    ...
    return $('div.line:lt('+end+')').slice(start-1);
};
